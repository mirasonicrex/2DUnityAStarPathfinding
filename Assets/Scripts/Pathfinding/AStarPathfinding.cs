...
public class AStarPathfinding
{
    ...
    public AStarPathfinding(Grid grid)
    {
        ...
        // cell size needs to be added to some calculations? 
    }
    ...
    private int CalculateDistance(Node a, Node b)
    {
        ...
        if (xDistance > yDistance)
        {
            ...
            // the cost may need to change since the cell size isn't one 
            return (int)(14 * diagonalSteps * grid.CellSize + 10 * straightSteps * b.Walkability);
        }
        else
        {
            int diagonalSteps = Mathf.Min(xDistance, yDistance);
    ...
    private Node GetLowestCostNode()
    {
       ...
       // UpdateVisualization(lowestCostNode);
    ...
    private List<Node> GetNeighbors(Node node)
    {
       ...
       // int x = (int)node.Position.x; // problem with converting to ints?
       ...
       //UpdateNeighborsVisualization(neighbors);
        return neighbors;
    }

    
    // Debugging only 
    private void UpdateNeighborsVisualization(List<Node> neighbors)
    {
        ...
        foreach (Node node in neighbors)
        {
            ...
            tile.GetComponent<SpriteRenderer>().color = new Color(1f, 0.36f, 0f, 0.43f);
    ...
    public void UpdateVisualization(Node node)
    {
        ...
        int y = Mathf.RoundToInt(gridPos.y);
        GameObject tile = grid.GridArray[x,y].tile;
        // set lowest cost node 
        tile.GetComponent<SpriteRenderer>().color = new Color(1f, 0.11f, 0.88f, 0.84f);
    }
